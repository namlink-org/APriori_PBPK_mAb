...
2. samples six drug/target parameters for each of eight mAbs and FcRn
   expression with 40% CV,
   plus endosomal clearance and whole-body lymph flow with 30% CV, using Latin
   hypercube sampling from log-normal distributions;
...
# Endosomal clearance is the first-order loss rate from vascular endosomes.
# In the original model it is calculated as:
#
#   endosomal uptake rate - endosomal recycling rate
#
# Sampling this parameter directly therefore overrides that formula for each
# virtual monkey. This is intentional here because the requested IIV represents
# variability in net lysosomal/endosomal loss beyond variability already
# encoded in the drug-specific parameters.
endosomal_clearance_source <- tibble(
  Drug = "Physiology",
  Target = "Endosomal system",
  Parameter = "Specific clearance (endosome)",
  Typical = 0.2052,
  CV = ENDOSOMAL_CL_CV,
  Parameter_class = "Physiology"
)

# Lymph flow is organ specific in PK-Sim. We sample one whole-body multiplier
# per monkey and apply it to every organ's baseline "Lymph flow
# proportionality factor." This preserves the physiological organ-to-organ
# pattern while adding correlated 30% IIV in overall lymphatic return.
#
# The baseline factors are read directly from the AntiPD1 monkey model rather
# than duplicated manually in this script.
trace_simulation <- loadSimulation(
  file.path(
    project_dir, "00_Models", "PKML_Files_Monkey",
    "PKML_Files_Monkey", "AntiPD1.pkml"
  ),
  loadFromCache = FALSE
)
lymph_parameters <- getAllParametersMatching(
  "Organism|*|Lymph flow proportionality factor",
  trace_simulation
)
lymph_baseline <- tibble(
  Path = map_chr(lymph_parameters, ~ .x$path),
  Typical = map_dbl(lymph_parameters, ~ .x$value),
  Units = map_chr(lymph_parameters, ~ .x$unit)
) %>%
  filter(!str_detect(Path, "EndogenousIgG"))
stopifnot(nrow(lymph_baseline) == 15L)

lymph_multiplier_source <- tibble(
  Drug = "Physiology",
  Target = "Lymphatic return",
  Parameter = "Whole-body lymph flow multiplier",
  Typical = 1,
  CV = LYMPH_FLOW_CV,
  Parameter_class = "Physiology"
)
...
container_map <- c(
  ...
  "Specific clearance (endosome)" = "Organism"
)
unit_map <- c(
  unit_map,
  "Start concentration of free FcRn (endosome)" = "µmol/l",
  "Specific clearance (endosome)" = "1/min"
)

# Physiological draws are shared across mAbs for a given virtual monkey.
physiology_source <- bind_rows(
  fcrn_source, endosomal_clearance_source, lymph_multiplier_source
)
...
which includes drug/target IIV, FcRn-expression IIV, endosomal-clearance IIV,
correlated whole-body lymph-flow IIV, sparse sampling, and 40% observation
error. The observed group is shifted by `log(2)`.
