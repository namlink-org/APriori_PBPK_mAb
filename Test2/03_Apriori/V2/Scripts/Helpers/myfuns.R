...
#' @return A data.frame with Time_hr, Conc_ngml, CompoundId, Species, Route
process_results <- function(results_df, model_params_path) {
  
  mw_table <- get_mw_table(model_params_path)
  
  # Rename columns for convenience
  conc_col <- grep("Plasma \\(Peripheral Venous Blood\\)", names(results_df), value = TRUE)
  names(results_df)[names(results_df) == conc_col] <- "Conc_umolL"
  names(results_df)[names(results_df) == "Time [min]"] <- "Time_min"
  
  # Ensure numeric types (columns may be read as character due to special characters)
  results_df$Conc_umolL <- as.numeric(results_df$Conc_umolL)
