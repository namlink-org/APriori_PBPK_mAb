...
metrics_out <- bind_rows(metrics_tbl, human_metrics_tbl) %>%
  ...
  rename(
    `Dose (mg/kg)`       = Dose,
    `Generic BW (kg)`    = BW_kg,
    `Obs Cmax (ug/mL)`   = Obs_Cmax,
    `Pred Cmax (ug/mL)`  = Pred_Cmax,
    `Fold Error Cmax`    = FE_Cmax,
    `Obs AUC0-28d (ug*h/mL)`  = Obs_AUC,
    `Pred AUC0-28d (ug*h/mL)` = Pred_AUC,
    `Fold Error AUC0-28d`     = FE_AUC,
    `Obs AUCinf (ug*h/mL)` = Obs_AUCinf,
    `Pred AUCinf (ug*h/mL)` = Pred_AUCinf,
    `Fold Error AUCinf` = FE_AUCinf,
    `Obs terminal CL (L/day)` = Obs_CL_L_day,
    `Pred terminal CL (L/day)` = Pred_CL_L_day,
    `Fold Error terminal CL` = FE_CL,
    `Obs mean profile conc (ug/mL)` = Obs_mean_profile_conc,
    `Pred mean profile conc (ug/mL)` = Pred_mean_profile_conc,
    `Mean pointwise fold error` = Mean_pointwise_FE,
    `GOF timepoints (n)` = GOF_timepoints_n
  ) %>%
  select(
    Species, Target, Molecule, `Dose (mg/kg)`, `Generic BW (kg)`,
    `Obs Cmax (ug/mL)`, `Pred Cmax (ug/mL)`, `Fold Error Cmax`,
    `Obs AUC0-28d (ug*h/mL)`, `Pred AUC0-28d (ug*h/mL)`, `Fold Error AUC0-28d`,
    `Obs AUCinf (ug*h/mL)`, `Pred AUCinf (ug*h/mL)`, `Fold Error AUCinf`,
    `Obs terminal CL (L/day)`, `Pred terminal CL (L/day)`, `Fold Error terminal CL`,
    `Obs mean profile conc (ug/mL)`, `Pred mean profile conc (ug/mL)`,
    `Mean pointwise fold error`, `GOF timepoints (n)`
  ) %>%
  mutate(across(where(is.numeric), ~ round(.x, 2)))

wb <- createWorkbook()
for (species_label in c("Monkey", "Human")) {
  sheet_name <- paste0(species_label, "_Metrics")
  species_out <- metrics_out %>% filter(Species == species_label)
  addWorksheet(wb, sheet_name)
  writeData(wb, sheet_name, species_out)
  setColWidths(wb, sheet_name, cols = seq_len(ncol(species_out)), widths = "auto")
}
